...
This is a blueprint for the full proof of the modularity conjecture:
...
\begin{theorem}
	...
	\uses{defn:Ell_curve_ap}
	 ...
	 Let $E$ be an elliptic curve over $\QQ$. Then there exists $N \in \mathbb{N}$ and a normalised
	 cuspidal eigenform $f \in S_{2}(\Gamma_0(N))$ such that for all primes $p$ with $p \nmid N$, we have $a_p(E)=a_p(f)$ where $f= \sum_n a_n(f)q^n$ is the $q$-expansion of $f$.
\end{theorem}
