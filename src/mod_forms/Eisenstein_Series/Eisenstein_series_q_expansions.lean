...
import number_theory.zeta_function
...
∑' (x : ℤ × ℤ),  1/(((x.1 : ℂ)*z+x.2)^k) = 2 * (Riemann_zeta k) +
  2 * (∑' (c : ℕ+), (∑' (d : ℤ), 1/(c*z + d)^k)) :=
begin
  rw [Riemann_zeta, tsum_prod, sum_int_even],
...
begin
  simp only [subtype.val_eq_coe, norm_eq_abs, absolute_value.map_mul, complex.abs_pow, abs_cast_nat,
    coe_coe, complex.abs_two],
  have hn := n.2.2,
  simp only [subtype.val_eq_coe, nat.mem_divisors_antidiagonal, ne.def, pnat.ne_zero, not_false_iff,
    and_true] at *,
  norm_cast,
  simp_rw ←hn,
...
lemma summable1 {k : ℕ} (z : ℍ)  :  summable (λ (p : Σ (b : ℕ+), ↥(nat.divisors_antidiagonal b)),
  ((sigma_antidiagonal_equiv_prod p).fst : ℂ) ^ k *
    exp (2 * ↑π * I * ↑z * ((sigma_antidiagonal_equiv_prod p).fst) *
      ((sigma_antidiagonal_equiv_prod p).snd))) :=
begin
...
lemma Eisenstein_Series_q_expansion (k : ℕ) (hk : 3 ≤ (k : ℤ)) (hk2 : even k) (z : ℍ) :
  ...
  2 * ((-2 *  ↑π * I)^k/(k-1)!) *
    ∑' (n : ℕ+),  (sigma_fn (k-1) (n))*(complex.exp ( 2 *↑π * I * z * n)) :=
begin
...
lemma Eisenstein_Series_q_expansion_Bernoulli (k : ℕ) (hk : 3 ≤ (k : ℤ)) (hk2 : even k) (z : ℍ) :
  (Eisenstein_Series_of_wt_ k) z =
...
begin
  ...
  obtain ⟨c, hc⟩ := hk2,
  have : k/2 = c,
    by {apply nat.div_eq_of_eq_mul_left,
    linarith,
    rw mul_comm,
    exact hc,},
  ...
  have hhk : 2*(k/2)=k, by {
    ...
    rw  ←even_iff_two_dvd,
    apply hke,},
  have H := riemann_zeta_two_mul_nat hk2_ne_0,
  simp_rw hhk at H,
  convert H.symm,
  norm_cast,
  exact hhk.symm,
...
begin
  ...
  have r1 : ((-2 *  ↑π * I)^4/(4-1)!)= (16 * π^4)/(3!),
    by {ring_exp, rw I_pow_4, simp only [one_mul],},
  rw r1 at H3,
  have r2 : ∀ (n : ℤ),
  complex.exp ( 2 *↑π * I * I * n) = complex.exp (-2 * π * n),
    ...
    simp only [neg_mul],
    congr,
    have : 2 * ↑π * I * I * ↑n = 2 * ↑π * (I * I) * ↑n,
      by {ring},
    rw this,
    rw I_mul_I,
    ring,},
  simp only [nat.cast_bit0, algebra_map.coe_one, nat.factorial_succ, nat.factorial_two,
    nat.cast_mul, nat.cast_add, nat.succ_sub_succ_eq_sub, tsub_zero, subtype.coe_mk] at H3,
  ...
  have H4 : 0 ≤ ∑' (n : ℕ+), (↑(sigma_fn 3 (n)) * real.exp (↑(-2 : ℤ) * π * ↑n)),
    by {apply tsum_nonneg, intro b, apply mul_nonneg, norm_cast, apply sigma_fn_nonneg,
