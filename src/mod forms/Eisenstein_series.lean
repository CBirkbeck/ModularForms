...
begin
rw Eise, rw Eise, rw moeb, simp, rw mat2_complex,   dsimp, rw ← coe_coe, rw ← coe_coe, rw calc_lem,
have h1:= coe_chain A, simp at h1, rw h1, rw h1, rw h1, rw h1, rw ← coe_coe,
simp,rw ← coe_coe, rw ← coe_coe, simp, refl,

have hh:= preserve_ℍ.aux A, apply hh, have:=A.2, norm_cast,
have kk:= det_coe_sl  A, rw kk, rw this, simp,simp only [subtype.coe_prop],
end
...
def lbpoint (A B : ℝ) (h: 0 < B): ℍ:= ⟨⟨A,B⟩, by {rw H_mem, simp, exact h,},⟩
...
have i6:= i3 i4 i5,
simp_rw i6, simp,  apply aux5, apply aux5, exact h, exact (H_mem z_val_val).mp z_val_property,
