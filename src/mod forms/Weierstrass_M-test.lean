...
rw ← complex.norm_eq_abs,
 simp_rw ← complex.norm_eq_abs,
  apply norm_tsum_le_tsum_norm, exact h,
end
...
begin
have Mpos: ∀ (n: ℕ), 0 ≤ M n , by {intro n,
have:= h1 n,
...
have HU: ∃ (a: ℕ), ∀ (b: ℕ), a ≤ b → ∥ ∑' i, M (i+b) ∥ < ε, by {
...
use HU_w,
intros b hb,  intro r,
 have:  ∑' i, complex.abs (F (i+b) r)  ≤ ∥ ∑' i, M (i+b) ∥, by {
   have r1:∥ ∑' i, M (i+b) ∥=∑' i, M (i+b), by {
 ...
 cases H, cases h2, dsimp at *,
have hut:=HU_h b hb, exact gt_of_gt_of_ge (HU_h b hb) this,},

have H2: ∀ (a: α) (k: ℕ),
∑' (n : ℕ), F n a - ∑ (i : ℕ) in finset.range k, F i a=∑' (n: ℕ), F (n+k) a, by {
intros a k, apply  sum_sub_tsum_nat_add k, exact hS a,

},
simp_rw dist_eq_norm,
simp_rw H2,
simp at *, cases BU, use BU_w, intros b hb r, have BUC:= BU_h b hb r,
let G:= (λ (i: ℕ), F (i) r),
have f_um:= abs_tsum (λ (i: ℕ), F (i+b) r) _, exact gt_of_gt_of_ge BUC f_um,
have f_sum:= (S1 r),
